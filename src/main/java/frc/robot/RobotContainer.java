...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    vision.setDefaultCommand(Commands.run(() -> vision.logAprilTagData(), vision));

    new EventTrigger("prepareElevatorL1").onTrue(Commands.runOnce(() -> {
      System.out.println("Raising elevator to level 1!");
    }));
  ...
  private void configureBindings() {
    ...
    // Right Bumper -> Run intake conditionally
    m_operatorController.rightBumper().whileTrue(
        Commands.either(
            Commands.either(
                m_funnelIntakeSubsystem.runIntakeCommand(), // If runFunnelIntake is true, run Funnel Intake
                Commands.either(
